...
def title(string, color, size=40):
    print("\033[{};1m{}\n{:^{}}\n{}\033[0m".format(color, '=' * size, string, size, '=' * size))
...
def run_exe(exe, reps, longest=0):
    ...
    sys.stdout.flush()
    for i in range(reps):
        print("\033[G\033[33m[ ] {:{}} [{}/{}]\033[0m".format(exe, spacing, i, reps), end='')
        sys.stdout.flush()
        start = time.time()
        result = subprocess.run([exe], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        end = time.time()
        diff = end - start
        max_t = max(max_t, diff)
        min_t = min(min_t, diff)
        avg += diff
    print("\033[G\033[32m[] {:{}} [{}/{}]\033[0m".format(exe, spacing, reps, reps))
...
def run(exes, files, reps, capture=[Capture.TIME, Capture.LOC]):
    ...
    for exe in exes:
        ...
        if Capture.LOC in capture and Capture.TIME in capture:
            data[language]['eff'] = data[language]['size']['bytes'] * data[language]['time']['avg']
    return data
...
def main():
    ...
    parser.add_argument(
        '--args',
        nargs='*',
        help='arguments to pass to the executables.')

    parser.add_argument(
        '-c', '--color', action='store_true', help='enables color in output.')
    parser.add_argument(
            '-g', '--graph', default=['bytes'], nargs='*', help='data sets to graph', choices={'min', 'max', 'avg', 'eff', 'bytes', 'lines'}
            )
    parser.add_argument(
            '-t', '--table', default=['color', 'name', 'avg', 'bytes', 'eff'], nargs='*', help='data sets to place in table', choices={'color', 'name', 'min', 'max', 'avg', 'eff', 'bytes', 'lines'}
            )
    ...
    # subtitle('Time', 35)
    # display_graph(format_graph_data(data, 'time.avg'))
    # subtitle('Efficiency', 35)
    # display_graph(format_graph_data(data, 'eff'), 2, 80, 40)
    # subtitle('Bytes of Code', 35)
    # display_graph(format_graph_data(data, 'size.bytes'), 2, 'max', 'max')
    # display_table(format_table_data(data, ['color', 'name', 'time.min', 'time.avg', 'time.max', 'size.bytes', 'eff']))
    display(data, args)
